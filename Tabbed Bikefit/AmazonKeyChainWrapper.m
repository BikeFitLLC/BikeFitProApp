...
+(void)storeValueInKeyChain:(NSString *)value forKey:(NSString *)key
{
    ...
    [keychainDictionary setObject:(__bridge id) kSecClassGenericPassword forKey:(id)kSecClass];
    ...
    [keychainDictionary setObject:(__bridge id) kSecAttrAccessibleWhenUnlockedThisDeviceOnly forKey:(id)kSecAttrAccessible];
...
+(OSStatus)wipeKeyChain
{
    ...
    keychainError = SecItemDelete((__bridge CFDictionaryRef)[AmazonKeyChainWrapper createKeychainDictionaryForKey : kKeychainUsernameIdentifier]);
    if(keychainError != errSecSuccess && keychainError != errSecItemNotFound)
    {
